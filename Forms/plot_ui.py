...
class PlotWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def redraw_ax1(self):
        self.ax1.clear()
        for i in self.project_controller.Assessments.columns:
            if i != "epoch":
                self.ax1.plot(self.project_controller.Assessments["epoch"].values,
                              self.project_controller.Assessments[i].values, label=i)
                self.ax1.scatter(self.project_controller.Assessments["epoch"].values,
                                 self.project_controller.Assessments[i].values)
    ...
    def redraw_ax2(self):
        ...
        for i in self.project_controller.Accuracies.columns:
            if i != "epoch":
                self.ax2.plot(self.project_controller.Accuracies["epoch"].values,
                              self.project_controller.Accuracies[i].values, label=i)
                self.ax2.scatter(self.project_controller.Accuracies["epoch"].values,
                                 self.project_controller.Accuracies[i].values)
    ...
    def redraw_ax4(self):
        self.ax4.clear()
        N = np.arange(0, self.project_controller.get("epoch_deFacto"))
        loss = self.project_controller.get("loss")
        val_loss = self.project_controller.get("val_loss")
        acc = self.project_controller.get("acc")
        val_acc = self.project_controller.get("val_acc")
    ...
    def refreshFigure(self, data: dict):
        if data.get("action") == "chr_plotting":
            ...
            # v = data.get("data")
            # vs = pd.DataFrame([v])
            # self.project_controller.Assessments = self.project_controller.Assessments.append(vs, ignore_index=True, sort=True)
            # self.ax1.clear()
            # for i in self.project_controller.Assessments.columns:
            #     if i != "epoch":
            #         self.ax1.plot(self.project_controller.Assessments["epoch"].values, self.project_controller.Assessments[i].values, label=i)
            #         self.ax1.scatter(self.project_controller.Assessments["epoch"].values, self.project_controller.Assessments[i].values)
            #         # https://matplotlib.org/3.3.2/api/_as_gen/matplotlib.pyplot.scatter.html
            #
            # # self.ax1.legend(bbox_to_anchor=(1.05, 1))
            # setOneAxesProperties(self.ax1, "Assessments - Epoch", "Epoch #", "Assessments")
            ...
            # self.project_controller.Accuracies = self.project_controller.Accuracies.append(add, ignore_index=True, sort=True)
            ...
            # for i in self.project_controller.Accuracies.columns:
            #     if i != "epoch":
            #         self.ax2.plot(self.project_controller.Accuracies["epoch"].values, self.project_controller.Accuracies[i].values, label=i)
            #         self.ax2.scatter(self.project_controller.Accuracies["epoch"].values, self.project_controller.Accuracies[i].values)
            #
            # setOneAxesProperties(self.ax2, "Accuracy - Epoch", "Epoch #", "Accuracy")
            # # self.ax2.legend(bbox_to_anchor=(1.05, 1))
            # self.canvas.draw()
            ...
            # self.project_controller.Params = self.project_controller.Params.append(adds, ignore_index=True, sort=True)

            # self.ax3.clear()
            # for i in self.project_controller.Accuracies.columns:
            #     if i != "epoch":
            #         self.ax3.plot(self.project_controller.Accuracies[i].values, self.project_controller.Params[i].values, label=i)
            #         self.ax3.scatter(self.project_controller.Accuracies[i].values, self.project_controller.Params[i].values)
            #
            # setOneAxesProperties(self.ax3, "Accuracy - Params", "Accuracy", "Params")
            ...
            # for i in self.project_controller.Params.columns:
            #     if i != "epoch":
            #         self.ax5.plot(self.project_controller.Params["epoch"].values, self.project_controller.Params[i].values, label=i)
            #         self.ax5.scatter(self.project_controller.Params["epoch"].values, self.project_controller.Params[i].values)
            # setOneAxesProperties(self.ax5, "Params - Epoch", "Epoch", "Params")
            # self.canvas.draw()
            pass
            ...
            self.project_controller.Assessments = pd.DataFrame(columns=columns)
