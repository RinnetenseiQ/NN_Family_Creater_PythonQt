    ...
    def __init__(self, d2d_rp: D2dRandomParams, neurons):
        ...
        if d2d_rp.dropoutExist:
            if sr.randrange(100) < 10:
                self.dropoutRate = sr.randint(10, d2d_rp.dropoutRange)
        self.actIndex = sr.randrange(d2d_rp.actIndexRange)

    def mutateActivation(self, mutateRate):
        if sr.randrange(100) <= mutateRate: self.actIndex = sr.randrange(self.d2d_rp.actIndexRange)
            ...
            if sr.randrange(100) < mutateRate:
                ...
                if way < 30:
                    if self.dropoutRate == 0:
                        self.dropoutRate = sr.randrange(1, self.d2d_rp.dropoutRange)
                    else:
                        self.dropoutRate == 0
                else:
                    self.dropoutRate = sr.randrange(1, self.d2d_rp.dropoutRange)
